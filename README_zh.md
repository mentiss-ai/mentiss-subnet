...
好人阵营玩家（预言家、女巫、猎人、村民）由 Mentiss 游戏引擎控制。每局游戏从平台的响应模型池中**随机选择一个模型**，分配给**所有**好人阵营 AI 玩家。这意味着每局游戏都是一场清晰的**"矿工 vs 单一模型"**对决。

所选模型**在不同游戏之间轮换** — 不同游戏会从模型池中抽取不同的模型（如 DeepSeek、Gemini、GLM 等）。模型池会定期进行健康检查，只有在延迟阈值内响应的模型才会被纳入。这种设计带来：

- **公平评估** — 每局游戏将矿工的技能隔离在单一已知对手模型下进行评估
- **跨游戏多样性** — 经过多局游戏，矿工将面对轮换的多种前沿模型
- **局内一致难度** — 所有 AI 对手在同一水平上进行游戏，消除混淆变量
